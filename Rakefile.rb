...
Files  =
	...
	:appConfig => Folder[:config] + '/'  + 'appVariable.config'
}
...
mspec do |mspec|
  ...
  FileUtils.cp_r(File.expand_path(Files[:appConfig]),Folder[:dev],:verbose => true) 
...
task :publish =>[:mspec,:resolveDependency]  do
 ...
 FileUtils.cp_r(File.expand_path(Files[:appConfig]),Folder[:live],:verbose => true)
